...
The trust boundary is drawn so that its core guarantees are **machine-checked
rather than review-checked** — enforced by the Rust type system or by
executable test vectors, not by discipline:

- **Secrets can't be logged.** The `Secret` type implements neither
  `Display`, `Serialize`, nor a value-revealing `Debug`; it isn't `Clone`.
...
| T2 | **SSRF / DNS rebinding** — an allowlisted name resolving to cloud metadata, loopback, or an internal address | The **guard** checks the *actual resolved IP* at dial time (after resolution), not the hostname at match time. Metadata + loopback are denied by default, so an allowlisted name pointing at `169.254.169.254` still fails at the socket. |
...
| T10 | **Trusting a spoofed upstream** | Upstream TLS is verified against the system roots (TLS ≥ 1.2), not disableable in v1. |
...
- **`warn` mode allows traffic by definition.** An `allowlist` in warn mode
  audits would-be rejections but lets them through; it is a staging aid, not
  a control.
...
3. **Scope the trust store.** Install the hematite CA only in the workloads
   that must trust it, not fleet-wide.
4. **Keep secrets out of config.** Config holds only the *name* of an env var
   or a file path — never a secret value. Provide the value via the
...
6. **Review `upstream_deny_cidrs`.** The defaults cover cloud metadata and
   ...
   workloads should not reach them — those are *not* denied by default.

## Reporting

This is a v0.1 reference implementation. Security-relevant findings should be
raised as issues (or via private disclosure to the maintainers) rather than
