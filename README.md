...
On Kubernetes, install the Helm chart at `deploy/chart/hematite/` — one
Deployment + Service per namespace, with an optional NetworkPolicy that locks
...
(the repository's **Security** tab → *Report a vulnerability*), not public
issues. The threat model and security properties are documented in
...
- **Squid** and **Envoy** — the contrast that shaped the approach: they
  enforce egress policy through configuration interpreted by general-purpose
  proxies. hematite instead makes the policy the program, with the security
  invariants carried by Rust's type system.
